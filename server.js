...
const MONGODB_URI =
  ...
  "mongodb+srv://PASSWORD_MANAGER:Pm%4012345@cluster0.7djwocg.mongodb.net/securevault?retryWrites=true&w=majority";
...
app.patch("/api/passwords/:id", auth, async (req, res) => {
  try {
    ...
    const entryIndex = req.user.passwordEntries.findIndex(
      (entry) => entry.id === id
    );
...
app.delete("/api/passwords/:id", auth, async (req, res) => {
  try {
    ...
    req.user.passwordEntries = req.user.passwordEntries.filter(
      (entry) => entry.id !== id
    );
