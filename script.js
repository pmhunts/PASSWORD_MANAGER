...
function loginUser(user, token) {
  ...
  // Attach event listeners for password manager buttons now that they are visible
  document
    .getElementById("addNewBtn")
    .addEventListener("click", addNewPasswordEntry);
  document
    .getElementById("exportBtn")
    .addEventListener("click", exportPasswordData);
...
function renderPasswordList() {
  ...
  filteredEntries.forEach((entry) => {
    ...
    showPasswordBtn.onclick = () =>
      togglePasswordVisibility(entry.id, showPasswordBtn);
    ...
    lastUpdatedCell.textContent = new Date(
      entry.lastUpdated
    ).toLocaleDateString();
  ...
  try {
    if (editingEntryId) {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/passwords/${editingEntryId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      // Update local array
      const entryIndex = passwordEntries.findIndex(
